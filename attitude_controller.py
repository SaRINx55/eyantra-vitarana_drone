...
class Edrone():
	...
	self.err_roll = error_roll()
	self.err_yaw = error_yaw()
	self.err_pitch = error_pitch()
        # initial setting of Kp, Kd and ki for [roll, pitch, yaw]. eg: self.Kp[2] corresponds to Kp value in yaw axis
        # after tuning and computing corresponding PID parameters, change the parameters
        self.Kp = [1.32, 1.32, 1900]
        self.Ki = [0.04, 0.04, 0.04]
        self.Kd = [112.5, 112.5, 39.3]
	...
	self.throttle_pwm = 0
        ...
        self.sample_time = 15  # in seconds

        # Publishing /edrone/pwm, /roll_error, /pitch_error, /yaw_error
        self.pwm_pub = rospy.Publisher('/edrone/pwm', prop_speed, queue_size=1)
        # ------------------------Add other ROS Publishers here-----------------------------------------------------
	self.error_roll_pub = rospy.Publisher('/roll_error', error_roll, queue_size=1)
	self.error_pitch_pub = rospy.Publisher('/pitch_error', error_pitch, queue_size=1)
	self.error_yaw_pub = rospy.Publisher('/yaw_error', error_yaw, queue_size=1)
        # -----------------------------------------------------------------------------------------------------------
...
#*************************************************************************************************************************************************
	...
	self.throttle_pwm = ((self.throttle - 1000)*128)/125
        #
        #
        #
        #
	...
	self.out_roll = self.Kp[0]*self.error[0] + self.Kd[0]*(self.error[0] - self.prev_error[0]) + self.Iterm[0]
	self.out_pitch = self.Kp[1]*self.error[1] + self.Kd[1]*(self.error[1] - self.prev_error[1]) + self.Iterm[1]
	self.out_yaw = self.Kp[2]*self.error[2] + self.Kd[2]*(self.error[2] - self.prev_error[2]) + self.Iterm[2]
	...
	self.err_roll.roll_error = self.out_roll
	self.error_roll_pub.publish(self.err_roll)

	self.err_pitch.pitch_error = self.out_pitch
	self.error_pitch_pub.publish(self.err_pitch)

	self.err_yaw.yaw_error = self.out_yaw
	self.error_yaw_pub.publish(self.err_yaw)

	print(self.out_roll)
	print(self.out_pitch)
	print(self.out_yaw)
